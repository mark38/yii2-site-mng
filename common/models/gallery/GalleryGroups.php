...
class GalleryGroups extends \yii\db\ActiveRecord
{
    ...
    {
        $images = GalleryImages::find()->where(['gallery_groups_id' => $this->id])->all();
        if ($images) {
            /** @var GalleryImages $image */
            foreach ($images as $image) {
                if (file_exists(Yii::getAlias('@frontend/web').$image->small)) unlink(Yii::getAlias('@frontend/web').$image->small);
                if (file_exists(Yii::getAlias('@frontend/web').$image->large)) unlink(Yii::getAlias('@frontend/web').$image->large);
            }
        }
